...
import copy
from fairlearn.reductions._exponentiated_gradient._constants import _PRECISION, _INDENTATION, _LINE
...
class _Lagrangian:
    ...
    def __init__(self, X, y, estimator, constraints, B, opt_lambda=True,
                 sample_weight_name='sample_weight', A = 0.0, **kwargs):
    ...
    def _eval2(self, X1, y1, sensitive_features, Q, lambda_vec):

        self.constraints1.load_data(X1, y1, sensitive_features = sensitive_features)
        self.obj1 = self.constraints1.default_objective()
        self.obj1.load_data(X1, y1, sensitive_features = sensitive_features)

        if callable(Q):
            error = self.obj1.gamma(Q)[0]
            gamma = self.constraints1.gamma(Q)

        #lambda_projected = self.constraints1.project_lambda(lambda_vec)
        #L_value = error + gamma.dot(lambda_projected)

        lambda_projected = self.constraints1.project_lambda(lambda_vec)
        L = error + np.sum(lambda_projected * (gamma - self.constraints1.bound()))
        violate = np.sum(lambda_projected * (gamma - self.constraints1.bound()))

        return L, error, violate
    ...
    def best_h(self, lambda_vec):
        ...
        h_gamma = self.constraints.gamma(h) - self.A
